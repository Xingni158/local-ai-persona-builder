...
function buildMessageWithMemory(message, history) {
  return `
...
Instruction:
Use the conversation history only when it is relevant. If the current question is unrelated, answer the current question directly.
`;
}
...
app.get("/", (req, res) => {
  res.json({
    ...
    memory: "session history supported",
...
app.post("/api/generate", async (req, res) => {
  try {
    ...
    const memoryMessage = buildMessageWithMemory(message, history);
    ...
    res.json({
      ...
      response,
      model: MODEL_NAME,
      memoryUsed: Array.isArray(history) && history.length > 0,
      historyTurnsReceived: Array.isArray(history) ? history.length : 0,
    });
  } catch (error) {
...
app.post("/api/compare", async (req, res) => {
  try {
    const { message, personas, history = [] } = req.body;
...
app.listen(PORT, () => {
  ...
  console.log("Memory: session history supported");
});