...
export default function App() {
  ...
  async function sendMessage() {
    ...
    setIsSending(true);
    setAiReply("Generating locally... The first response may take a few seconds.");
    ...
    try {
      ...
      if (data.response) {
        ...
        setAiReply("No response received from the local model.");
      }
    } catch (err) {
      console.error(err);
      setAiReply(
        ...
        <div className="mx-auto max-w-7xl rounded-[32px] border border-white/10 bg-white/[0.06] backdrop-blur-2xl shadow-2xl overflow-hidden">
          <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] min-h-[760px]">
            ...
            <main className="p-8 md:p-12">
              ...
              >
                {step === 0 && (
                  <Panel title="Install Ollama" tag="local runtime">
                    <p className="text-white/60 max-w-2xl mb-8">
                      Ollama provides the local model runtime. Install it first,
                      then pull a fast demo model such as {DEFAULT_MODEL}.
                    </p>
                    ...
                    <CommandBlock
                      ...
                      onCopy={() =>
                        copy(`brew install ollama\nollama pull ${DEFAULT_MODEL}`, "install")
                      }
                    />
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="inline-block mt-6 text-[#B7FF7A] hover:underline"
                    >
                      Open Ollama website →
                    </a>
                  </Panel>
                ...
                {step === 2 && (
                  <Panel title="Start Local Server" tag="terminal commands">
                    ...
                    <div className="space-y-5">
                      ...
                      <CommandBlock
                        command={`cd ~/Desktop/local-llm-project/backend\nMODEL_NAME=${DEFAULT_MODEL} PORT=3001 npm start`}
                ...
                {step === 5 && (
                  <Panel title="Launch Local Chat" tag="private session">
                    <div className="rounded-[28px] border border-white/10 bg-black/35 p-6 max-w-3xl">
                      ...
                      <div className="mt-6 rounded-2xl bg-white/[0.04] border border-white/10 p-5 text-white/70 min-h-[160px] whitespace-pre-wrap leading-relaxed">
                        {aiReply || "Local response will appear here."}
                      </div>
                    </div>
                  </Panel>
                )}
